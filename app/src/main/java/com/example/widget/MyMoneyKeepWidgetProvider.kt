...
class MyMoneyKeepWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        ...
        private fun getLocalizedResources(context: Context, languageCode: String): Resources {
            val locale = LocaleHelper.getLocale(languageCode)
    ...
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                android.util.Log.e("WidgetProvider", "Error updating widgets in onUpdate: ${e.message}", e)
            } finally {
    ...
    private suspend fun renderWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
        try {
            ...
            // 2. 本地化資源 (不觸發系統層級 Locale 廣播)
            val localizedRes = getLocalizedResources(context, langCode)
            ...
            // 設定多國語系文字標籤
            views.setTextViewText(R.id.widget_title, localizedRes.getString(R.string.widget_title))
            views.setTextViewText(R.id.widget_label_today_expense, localizedRes.getString(R.string.widget_today_expense_label))
            views.setTextViewText(R.id.widget_label_today_income, localizedRes.getString(R.string.widget_today_income_label))
            views.setTextViewText(R.id.widget_label_month_balance, localizedRes.getString(R.string.widget_month_balance_label))
            views.setTextViewText(R.id.widget_btn_calc, localizedRes.getString(R.string.widget_btn_calc_action))
            views.setTextViewText(R.id.widget_btn_record, localizedRes.getString(R.string.widget_btn_record_action))
